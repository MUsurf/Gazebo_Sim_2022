...
class ThrustAllocator():
    ...
    def __init__(self):
        ...
        self.wrench_subscriber = rospy.Subscriber('/jelly/controller/command_wrench', WrenchStamped, self.input_real_wrench)
    ...
    def publish_thruster_forces(self):
        ...
        self.message.layout.dim.label = "Label"
        ...
        for j in range(0,8):
                self._data[j] = self.get_input_value(self.thruster_forces[j],j)
        print(self._data)
        # for k in range(len(self.thruster_forces)):
            # self._frame_id = self.message.layout.dim.label[k]
        self.message.data = self._data
        self.publishers.publish(self.message)

    def get_input_value(self, force, thruster_id):
        if thruster_id == 0 or thruster_id == 7:
            active_thruster_output = []
            for i in self.output_values:
                # reversing the list
                active_thruster_output.insert(0, -i)
        elif thruster_id == 1 or thruster_id == 2 or thruster_id == 5 or thruster_id == 6:
            active_thruster_output = []
            for j in self.output_values:
                active_thruster_output.append(j / 2.2231)
        else:
            active_thruster_output = self.output_values
        if force < active_thruster_output[0]:
            self.thruster_input = 1150
            print("Too low")
            return self.thruster_input
        elif force > active_thruster_output[len(active_thruster_output) - 1]:
            ...
            for l in range(0,len(self.input_values)):
                if force < active_thruster_output[l]:
                    output_inter_high = active_thruster_output[l]
                    output_inter_low = active_thruster_output[l-1]
                    break
                l+=1
