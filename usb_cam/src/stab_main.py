...
class stabMain():
    ...
    def check_time(self):
        ...
        while not rospy.is_shutdown():
            ...
            elif (rospy.get_time() - self.start_time) >= 1.0:
                if self.flag.data == 0:
                    pkg = 'jelly_description'
                    print(type(pkg))
                    node_control = 'custom_controller_stab.py'
                    print(type(node_control))
                    node_thrust = 'custom_thrust_allocator_stab.py'
                    print(type(node_thrust))
                    controller_node = roslaunch.core.Node(package=pkg,node_type='executable',name=node_control)
                    print(type(controller_node))
                    thrust_allocator_node = roslaunch.core.Node(package=pkg,node_type='executable',name=node_thrust)

                    launch = roslaunch.scriptapi.ROSLaunch()
                    launch.start()

                    process = launch.launch(controller_node)
